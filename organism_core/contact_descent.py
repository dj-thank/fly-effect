"""Live-contact residual descent over every non-root DOF (FE-02).

The fixed-geometry descent proved the saved contact map itself is the
binding constraint: with all 66 non-root DOFs free, the subsystem residual
floors nonzero under a frozen contact map, and its optima do not transfer
to real contact geometry. This module removes that approximation — every
probe re-derives first-contact height, the depth grid, contacts, tendon
limits and the subsystem LP. The descent therefore explores the true
piecewise residual landscape, including contact-set changes (tarsus points
appearing or leaving the floor between probes).

Phase 1 descends; phase 2 certifies the generated terminal poses through
the unchanged full gate chain. Static witness generation only — no dynamic
hold, standing, walking, flight, CNS execution or biological validation.
"""
...
from .residual_descent import _certify, _residual

LEGS = ('lf', 'lm', 'lh', 'rf', 'rm', 'rh')
PROTOCOL = {
    'id': 'FE-02-contact-descent-v1',
    ...
    'objective': ('passive-subsystem minimum scaled balance error with LIVE '
                  'contact re-derivation per probe: first-contact height, '
                  'depth grid, contacts, tendon limits and bias target are '
                  'all recomputed at each trial pose; the residual is the '
                  'minimum over root-feasible depth samples'),
    ...
    'max_solver_calls_per_seed': 40000,
    ...
    'scope': ('Static witness generation; contacts are re-derived per probe '
              'but this is a bounded declared search, not global posture '
              'optimization; no dynamic or biological claim'),
}
WITNESS = re.compile(r'^(cdesc_\d+_cert_\d+_depth_\d+_)support_tendon_map$')
...
def _probe(body, transmission, q, fmax, l0, passive, deadline):
    """Evaluate the subsystem residual at one pose with live contacts.

    Mirrors the gate chain's per-iterate evaluation: fresh first-contact
    height, the declared depth grid, contact detection, the root-balance LP,
    then the subsystem LP on every root-feasible sample. The residual is the
    minimum over root-feasible samples; +inf when no sample is eligible.
    Returns (R, status, detail). Saves nothing — callers record evidence.
    """
    import mujoco as mj
    ...
    best = (float('inf'), 'no_root_feasible_sample', None)
    for n, depth in enumerate(PROTOCOL['depths_native']):
        ...
        if np.any(data.warning.number):
            raise ValueError('Solver warning in live-descent probe')
        ...
        if not contacts or nonfoot:
            continue
        rb = root_balance(C[:6], (data.qfrc_bias-data.qfrc_passive)[:6], mu)
        calls['solver'] += 1
        if rb.get('feasible') is not True:
            continue
        limits = fmax*np.exp(-((data.ten_length/l0-1)/.5)**2)
        _, inputs = evaluate_support(body, transmission, limits)
        calls['solver'] += 1
        sub = solve_subsystem(*(inputs[k] for k in FIELDS[:4]),
                              inputs['support_friction'],
                              list(range(6))+passive)
        calls['solver'] += 1
        if sub['status'] == 'feasible':
            R, st = 0., 'feasible'
        elif (sub['status'] == INFEASIBLE and sub.get('nearest_balance')
                and np.isfinite(sub['nearest_balance'].get(
                    'minimum_scaled_balance_error', np.inf))):
            R = float(sub['nearest_balance']['minimum_scaled_balance_error'])
            st = 'infeasible'
        else:
            R, st = float('inf'), sub.get('status', 'unresolved')
        if R < best[0] or best[2] is None:
            best = (R, st, {'index': n, 'depth_native': depth,
                            'foot_legs': legs, 'subsystem_status': st,
                            'inputs': inputs,
...
def _descend_live(body, transmission, fmax, l0, passive, dof_rows, dof_adrs,
                  dof_ranges, q_seed, deadline, budget):
    """Cyclic coordinate descent on the live-contact subsystem residual.

    Every accepted step saves the full LP inputs of its driver sample so the
    verifier replays without physics. Probes returning +inf (no root-feasible
    sample or unresolved LP) are rejected automatically.
    """
    q = np.asarray(q_seed, dtype=float).copy()
    R, st, detail, calls = _probe(body, transmission, q, fmax, l0, passive,
                                  deadline)
    budget[0] -= 1
        ...
        for di, (adr, (lo, hi)) in enumerate(zip(dof_adrs, dof_ranges)):
            ...
            for sign in (1., -1.):
                ...
                if qp[adr] == q[adr]:
                    continue
                try:
                    Rp, stp, detp, calls = _probe(body, transmission, qp,
                                                  fmax, l0, passive, deadline)
...
def run(workspace):
    """Descend the live subsystem residual, then certify terminal poses."""
    import mujoco as mj
    ...
    out = workspace/'contact-descent'; out.mkdir(exist_ok=False)
    ...
    try:
        record_path = (Path(__file__).resolve().parents[1]
                       /'examples/fe02-contact-descent.json')
        ...
        for si, seed in enumerate(seeds):
            ...
            try:
                drec, acc_arrays, q_term, detail = _descend_live(
                    body, transmission, fmax, l0, passive, dof_rows, dof_adrs,
...
def _check_descent_replay(si, srec, arrays, rows):
    """Replay every saved accepted-step subsystem LP."""
    errs = []
    undecided = False
    dp = f'cdesc_{si:02d}_'
    ...
    for a in accepted:
        ...
        if sp+'support_tendon_map' not in arrays:
            ...
            continue
        R_replay, st = _residual(arrays[sp+'support_tendon_map'],
...
def verify(workspace):
    """Replay the live-descent trajectory and every certification LP."""
    from .root_ab import file_hash, write_json
    from .root_ab_evidence import read_json
    from .static_support import solve_support
    workspace = Path(workspace).resolve()
    d = workspace/'contact-descent'
        ...
        for si, srec in enumerate(result.get('seeds', [])):
            ...
            for ci, crec in enumerate(crecs):
                for sample in crec['samples']:
                    n = sample['index']
                    prefix = f'cdesc_{si:02d}_cert_{ci:02d}_depth_{n:02d}_'
