"""Live-contact residual descent including root orientation (FE-02).

The live-contact descent over all 66 non-root DOFs floored nonzero. The
only unexplored posture axis is root orientation: root x,y are dead by
translation symmetry, root z is re-derived per probe, but tilting the
body can bring different tarsus points into the contact set. This module
repeats the live-contact descent with three additional world-frame
rotation DOFs (roll/pitch/yaw small-angle increments, total tilt from
the seed orientation bounded) on top of the 66 joint DOFs.

Phase 1 descends; phase 2 certifies generated poses through the
unchanged gate chain. Static witness generation only — no dynamic hold,
standing, walking, flight, CNS execution or biological validation.
"""
...
from .contact_descent import _probe

LEGS = ('lf', 'lm', 'lh', 'rf', 'rm', 'rh')
ROT_AXES = (np.array([1., 0., 0.]), np.array([0., 1., 0.]),
            np.array([0., 0., 1.]))
PROTOCOL = {
    'id': 'FE-02-root-descent-v1',
    ...
    'objective': ('passive-subsystem minimum scaled balance error with LIVE '
                  'contact re-derivation per probe, identical to '
                  'FE-02-contact-descent-v1'),
    ...
    'max_solver_calls_per_seed': 30000,
...
WITNESS = re.compile(r'^(rdesc_\d+_cert_\d+_depth_\d+_)support_tendon_map$')
...
def _tilt(q, q_seed):
    """Angle between the seed and current root orientations."""
    dot = min(1., abs(float(np.dot(q[3:7], q_seed[3:7]))))
    return 2.*np.arccos(dot)


def _descend_root(body, transmission, fmax, l0, passive, dof_rows, dof_adrs,
...
def run(workspace):
    """Descend the live residual over joints plus root tilt, then certify."""
    import mujoco as mj
    ...
    out = workspace/'root-descent'; out.mkdir(exist_ok=False)
    ...
    try:
        record_path = (Path(__file__).resolve().parents[1]
                       /'examples/fe02-root-descent.json')
        ...
        for si, seed in enumerate(seeds):
            ...
            try:
                drec, acc_arrays, q_term, detail = _descend_root(
...
def verify(workspace):
    """Replay the root-descent trajectory and every certification LP."""
    from .root_ab import file_hash, write_json
    from .root_ab_evidence import read_json
    from .static_support import solve_support
    workspace = Path(workspace).resolve()
    d = workspace/'root-descent'
        ...
        for si, srec in enumerate(result.get('seeds', [])):
            ...
            for ci, crec in enumerate(crecs):
                for sample in crec['samples']:
                    n = sample['index']
                    prefix = f'rdesc_{si:02d}_cert_{ci:02d}_depth_{n:02d}_'
