...
![Fly Effect](assets/fly-effect.svg)
...
計算基盤における生命や意識そのものの成立への到達を目指し、実現可能性と必要な条件を研究します。記憶・恒常性・自己維持は、その目標を置き換える終点ではなく中間段階です。現在のソフトウェアで成立したという主張ではありません。[研究計画](ORGANISM_CAPABILITIES.ja.md)
...
**現在はpre-alphaです。自律歩行・学習・飛行が完成した生物シミュレーターではありません。** ローカル研究版では六脚の接地反復を確認しましたが、3秒で約0.58mmの正味移動にとどまり、歩行条件を満たしていません。公開コードの検証、人工回路テスト、過去の研究結果は区別して報告します。[検証の範囲](STATUS.md)
...
人工回路のデモは導入・保存・再開の確認用で、ハエの再現性能の証拠ではありません。生物データ、メッシュ、学習済み重みは同梱しません。全CNS・身体実験には追加のデータと準備が必要です。[導入手順](GETTING_STARTED.md) / [データ](DATA.md)
...
- **有益な方向へ。** 科学的理解と人の暮らしに役立つ可能性を育てる。未検証の意識・完全な生物再現・臨床上の有効性を成立済みと表記しない。

これらは運営理念であり、ソフトウェアのライセンスに追加の用途制限を設けるものではありません。[理念](VISION.md)
...
歩行スクリプトや学習済み方策は診断・校正の教師には使えますが、実神経回路による自律歩行の成功と混同しません。[ロードマップ](ROADMAP.md)

## 参加方法

バグの再現、文書の改善、小規模モデル、単位・座標のレビュー、可視化、翻訳を歓迎します。大きな実験や設計変更はIssueで仮説と計算予算を共有してください。

[最初の貢献](FIRST_CONTRIBUTIONS.md) · [貢献ガイド](../CONTRIBUTING.md) · [行動規範](../CODE_OF_CONDUCT.md) · [運営](../GOVERNANCE.md) · [セキュリティ](../SECURITY.md)
